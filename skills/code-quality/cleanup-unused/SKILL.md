...
3. Uninstall deps using the project's package manager (detect from lockfile: `package-lock.json` → `npm uninstall`, `yarn.lock` → `yarn remove`, `pnpm-lock.yaml` → `pnpm remove`, `bun.lockb` → `bun remove`). For Python: `pip uninstall <pkg>` / `uv remove <pkg>` / `poetry remove <pkg>`. For Rust: `cargo remove <pkg>`.
