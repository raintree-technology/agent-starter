# Supabase Integration Expert Full Guide
...
- [Real-time Subscriptions](#real-time-subscriptions)
  ...
  - [Filter Real-time Events](#filter-real-time-events)
  - [Broadcast (Ephemeral Messages)](#broadcast-ephemeral-messages)
  - [Presence Tracking](#presence-tracking)
- [Storage](#storage)
  - [Upload Files](#upload-files)
  - [Download Files](#download-files)
  - [Public URLs](#public-urls)
  - [Signed URLs (Private Files)](#signed-urls-private-files)
...
- [Vector Search (AI/ML)](#vector-search-aiml)
  - [Enable pgvector](#enable-pgvector)
  - [Store Embeddings](#store-embeddings)
  - [Similarity Search](#similarity-search)
- [Next.js Integration](#nextjs-integration)
  - [Server Components](#server-components)
  - [Client Components](#client-components)
  - [Middleware (Auth Protection)](#middleware-auth-protection)
...
- [TypeScript Integration](#typescript-integration)
  ...
  - [Server vs Client Supabase](#server-vs-client-supabase)
- [Next.js App Router Patterns](#nextjs-app-router-patterns)
  ...
  - [Client Component with Real-time](#client-component-with-real-time)
  - [Route Handlers](#route-handlers)
- [Advanced Authentication](#advanced-authentication)
  - [Email/Password with Email Confirmation](#emailpassword-with-email-confirmation)
  - [OAuth (Google, GitHub, etc.)](#oauth-google-github-etc)
  - [Magic Links](#magic-links)
  - [Phone Auth (SMS)](#phone-auth-sms)
  - [Multi-Factor Authentication (MFA)](#multi-factor-authentication-mfa)
  - [Auth Callback Handler](#auth-callback-handler)
  - [Protected Routes](#protected-routes)
- [Advanced Row Level Security](#advanced-row-level-security)
  - [Complex RLS Policies](#complex-rls-policies)
  - [Function-Based RLS](#function-based-rls)
  - [RLS with JWT Claims](#rls-with-jwt-claims)
- [Advanced Real-time Features](#advanced-real-time-features)
  - [Presence (Who's Online)](#presence-whos-online)
  - [Broadcast (Send Messages)](#broadcast-send-messages)
  - [Postgres Changes (Database Events)](#postgres-changes-database-events)
...
Provide comprehensive, accurate guidance for building applications with Supabase using the target app's installed packages, database schema, local docs when pulled, and current official Supabase documentation. Cover database operations, authentication, realtime features, file storage, edge functions, vector search, and platform integrations without freezing package names or dashboard details that change over time.

## Documentation Coverage

**Full access to official Supabase documentation when pulled locally:**
- **Location:** `docs/`
- **Coverage:** Guides, API references, client libraries, and platform docs from the docs snapshot pulled for the target environment

**Note:** Documentation must be pulled separately:
```bash
pipx install docpull
docpull https://supabase.com/docs -o <installed-skill-dir>/docs
```
...
1. **Search for specific topics:**
   ...
   grep -r "row level security" docs/ --include="*.md"
   ```

2. **Find guides:**
   ```bash
   # Docs pullers can choose different filenames; inspect matches
   grep -r "Row Level Security" docs/ --include="*.md"
   ```
...
3. **Check reference docs:**
   ...
   grep -r "createClient" docs/ --include="*.md"
   ```
...
Use `@supabase/ssr` for current Next.js SSR patterns. Older Next.js auth-helper packages appear in legacy projects and migration guides; do not introduce them into new code unless the target app already depends on them and the task is explicitly a legacy maintenance patch.
...
```typescript
// proxy.ts or middleware.ts, depending on the target Next.js version
...
Protect server-rendered pages with `supabase.auth.getClaims()` or another server-validated authorization check. Do not trust `getSession()` alone in server code.
...
Search the local documentation snapshot:
...
# Find guides
grep -r "guide title" docs/ --include="*.md"

# Find API reference
grep -r "reference topic" docs/ --include="*.md"
```
