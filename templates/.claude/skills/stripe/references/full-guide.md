# Stripe Integration Expert Full Guide
...
- [Payment Integration Patterns](#payment-integration-patterns)
  - [Pattern 1: Stripe Checkout (Fastest)](#pattern-1-stripe-checkout-fastest)
  - [Pattern 2: Payment Element (Custom UI)](#pattern-2-payment-element-custom-ui)
  - [Pattern 3: Subscriptions](#pattern-3-subscriptions)
- [Webhook Implementation](#webhook-implementation)
  - [Setup Webhook Endpoint](#setup-webhook-endpoint)
- [Customer Management](#customer-management)
- [Advanced Features](#advanced-features)
  - [Connect (Marketplaces & Platforms)](#connect-marketplaces-platforms)
...
- [TypeScript Types & Configuration](#typescript-types-configuration)
...
- [Next.js App Router Integration](#nextjs-app-router-integration)
  - [Server Actions for Payments](#server-actions-for-payments)
  - [Client Component with Server Actions](#client-component-with-server-actions)
...
- [Stripe Connect Deep Dive](#stripe-connect-deep-dive)
  - [Account Creation & Onboarding](#account-creation-onboarding)
  - [Marketplace Payment Flows](#marketplace-payment-flows)
  - [Multi-Party Payments (Split Payments)](#multi-party-payments-split-payments)
  - [Connect Webhooks](#connect-webhooks)
- [Stripe Terminal (In-Person Payments)](#stripe-terminal-in-person-payments)
  - [Reader Setup](#reader-setup)
  - [Payment Collection](#payment-collection)
  - [Client-Side Terminal Integration](#client-side-terminal-integration)
- [Stripe Radar (Fraud Prevention)](#stripe-radar-fraud-prevention)
  - [Custom Fraud Rules](#custom-fraud-rules)
  - [3D Secure Authentication](#3d-secure-authentication)
  - [Reviewing Disputes](#reviewing-disputes)
- [Stripe Identity](#stripe-identity)
  - [Identity Verification](#identity-verification)
- [Stripe Issuing (Card Creation)](#stripe-issuing-card-creation)
  - [Create Virtual/Physical Cards](#create-virtualphysical-cards)
  - [Authorization Controls](#authorization-controls)
- [Stripe Tax](#stripe-tax)
  - [Automatic Tax Calculation](#automatic-tax-calculation)
- [Stripe Treasury (Banking-as-a-Service)](#stripe-treasury-banking-as-a-service)
  - [Financial Accounts](#financial-accounts)
- [Stripe Climate](#stripe-climate)
  - [Carbon Removal Contributions](#carbon-removal-contributions)
- [Metered Billing & Usage-Based Pricing](#metered-billing-usage-based-pricing)
  - [Setup Usage-Based Subscription](#setup-usage-based-subscription)
  - [Real-World Usage Tracking](#real-world-usage-tracking)
- [Billing Portal Customization](#billing-portal-customization)
  - [Create Custom Billing Portal](#create-custom-billing-portal)
- [Testing & Development](#testing-development)
  - [Test Mode Helpers](#test-mode-helpers)
  - [Test Scenarios](#test-scenarios)
- [Performance Optimization](#performance-optimization)
  - [Expand Related Objects](#expand-related-objects)
...
Provide comprehensive, accurate guidance for integrating Stripe payment infrastructure using the target app's installed SDKs, local docs when pulled, and current official Stripe documentation. Cover payment processing, billing, subscriptions, webhooks, fraud prevention, and advanced features without hardcoding fast-moving API-version details.

## Documentation Coverage

**Full access to official Stripe documentation when pulled locally:**
- **Location:** `docs/`
- **Coverage:** API reference, guides, integrations, and best practices from the docs snapshot that was pulled for the target environment

**Note:** Documentation must be pulled separately:
```bash
pipx install docpull
docpull https://docs.stripe.com -o <installed-skill-dir>/docs
```
...
1. **Search for specific topics:**
   ...
   grep -r "payment intent" docs/ --include="*.md"
   ```

2. **Read specific API references:**
   ```bash
   # API reference paths depend on the docs puller; inspect matches before reading
   grep -r "Create a PaymentIntent" docs/ --include="*.md"
   ```

3. **Find integration guides:**
   ```bash
   # Guides and tutorials
   find docs -maxdepth 2 -type f -name "*.md" | head
   ```
...
Do not hardcode an API version from an example. For `stripe-node` v12 and newer, the library aligns requests with the API version current when that SDK version was released. Pin or override `apiVersion` only after checking the installed Stripe SDK, the account default version in Workbench, and the webhook endpoint version.
...
Use Grep to search the local documentation snapshot:
...
grep -r "search term" docs/api/

# Find specific endpoint
ls docs/api/payment_intents/
```

**Common doc areas to search:**
- API reference
- Payment Intents
